  ...
  if (typeof allowedAttrs !== "undefined") {
    ...
    while (i--) {
      ...
      if (
        ...
        if (
          attrName.startsWith("se:") ||
          attrName.startsWith("oi:") ||
          attrName.startsWith("data-")
        ) {
          // We should bypass the namespace aswell
          const seAttrNS = attrName.startsWith("se:")
            ? NS.SE
            : attrName.startsWith("oi:")
            ? NS.OI
            : null;
          ...
          console.warn(
            `sanitizeSvg: attribute ${attrName} in element ${node.nodeName} not in whitelist is removed`
          );
    ...
    if (
      ...
      setHref(node, "");
      console.warn(
        `sanitizeSvg: attribute href in element ${node.nodeName} pointing to a non-local reference (${href}) is removed`
      );
    ...
    if (node.nodeName === "use" && !getHref(node)) {
      console.warn(
        `sanitizeSvg: element ${node.nodeName} without a xlink:href is removed`
      );
      node.remove();
      return;
    }
      ...
      (attr) => {
        ...
        if (val) {
          ...
          if (val && val[0] !== "#") {
            ...
            console.warn(
              `sanitizeSvg: attribute ${attr} in element ${node.nodeName} pointing to a non-local reference (${val}) is removed`
            );
    ...
    console.warn(
      `sanitizeSvg: element ${node.nodeName} not supported is removed`
    );
    const children = [];
